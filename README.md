...
The installer links agents, skills, hooks, commands, and scripts into `~/.claude/`, where Claude Code loads extensions from. Pick `--symlink` to get updates via `git pull`, or run without it for a stable copy.
...
Skills enforce methodology. They are not suggestions -- they are phased workflows with gates that prevent skipping steps.
...
Event-driven automation that fires on session start, before/after tool use, at compaction, and on stop. Error learning, context injection, quality enforcement, anti-rationalization -- all automatic.
...
- **Zero-expertise operation** -- The system should require no specialized knowledge from the user. Say what you want done. The system handles the rest.
